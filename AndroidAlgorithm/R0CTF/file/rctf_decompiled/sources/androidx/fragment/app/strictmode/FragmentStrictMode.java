...
public final class FragmentStrictMode {
    ...
    public static final void onFragmentReuse(Fragment fragment, String previousFragmentId) {
        ...
        Policy policy = fragmentStrictMode.getNearestPolicy(fragment);
        if (policy.getFlags$fragment_release().contains(Flag.DETECT_FRAGMENT_REUSE) && fragmentStrictMode.shouldHandlePolicyViolation(policy, fragment.getClass(), violation.getClass())) {
            fragmentStrictMode.handlePolicyViolation(policy, violation);
        }
    }
    ...
    public static final void onFragmentTagUsage(Fragment fragment, ViewGroup container) {
        ...
        fragmentStrictMode.logIfDebuggingEnabled(violation);
        Policy policy = fragmentStrictMode.getNearestPolicy(fragment);
        if (policy.getFlags$fragment_release().contains(Flag.DETECT_FRAGMENT_TAG_USAGE) && fragmentStrictMode.shouldHandlePolicyViolation(policy, fragment.getClass(), violation.getClass())) {
            fragmentStrictMode.handlePolicyViolation(policy, violation);
        }
    }
    ...
    public static final void onSetRetainInstanceUsage(Fragment fragment) {
        ...
        fragmentStrictMode.logIfDebuggingEnabled(violation);
        Policy policy = fragmentStrictMode.getNearestPolicy(fragment);
        if (policy.getFlags$fragment_release().contains(Flag.DETECT_RETAIN_INSTANCE_USAGE) && fragmentStrictMode.shouldHandlePolicyViolation(policy, fragment.getClass(), violation.getClass())) {
            fragmentStrictMode.handlePolicyViolation(policy, violation);
        }
    }
    ...
    public static final void onSetUserVisibleHint(Fragment fragment, boolean isVisibleToUser) {
        ...
        fragmentStrictMode.logIfDebuggingEnabled(violation);
        Policy policy = fragmentStrictMode.getNearestPolicy(fragment);
        if (policy.getFlags$fragment_release().contains(Flag.DETECT_SET_USER_VISIBLE_HINT) && fragmentStrictMode.shouldHandlePolicyViolation(policy, fragment.getClass(), violation.getClass())) {
            fragmentStrictMode.handlePolicyViolation(policy, violation);
        }
    }
    ...
    public static final void onSetTargetFragmentUsage(Fragment violatingFragment, Fragment targetFragment, int requestCode) {
        ...
        FragmentStrictMode fragmentStrictMode = INSTANCE;
        fragmentStrictMode.logIfDebuggingEnabled(violation);
        Policy policy = fragmentStrictMode.getNearestPolicy(violatingFragment);
        if (policy.getFlags$fragment_release().contains(Flag.DETECT_TARGET_FRAGMENT_USAGE) && fragmentStrictMode.shouldHandlePolicyViolation(policy, violatingFragment.getClass(), violation.getClass())) {
            fragmentStrictMode.handlePolicyViolation(policy, violation);
        }
    }
    ...
    public static final void onGetTargetFragmentUsage(Fragment fragment) {
        ...
        fragmentStrictMode.logIfDebuggingEnabled(violation);
        Policy policy = fragmentStrictMode.getNearestPolicy(fragment);
        if (policy.getFlags$fragment_release().contains(Flag.DETECT_TARGET_FRAGMENT_USAGE) && fragmentStrictMode.shouldHandlePolicyViolation(policy, fragment.getClass(), violation.getClass())) {
            fragmentStrictMode.handlePolicyViolation(policy, violation);
        }
    }
    ...
    public static final void onWrongFragmentContainer(Fragment fragment, ViewGroup container) {
        ...
        fragmentStrictMode.logIfDebuggingEnabled(violation);
        Policy policy = fragmentStrictMode.getNearestPolicy(fragment);
        if (policy.getFlags$fragment_release().contains(Flag.DETECT_WRONG_FRAGMENT_CONTAINER) && fragmentStrictMode.shouldHandlePolicyViolation(policy, fragment.getClass(), violation.getClass())) {
            fragmentStrictMode.handlePolicyViolation(policy, violation);
        }
    }
    ...
    private final boolean shouldHandlePolicyViolation(Policy policy, Class<? extends Fragment> fragmentClass, Class<? extends Violation> violationClass) {
        Set violationsToBypass = policy.getMAllowedViolations$fragment_release().get(fragmentClass);
    ...
    private final void handlePolicyViolation(final Policy policy, final Violation violation) {
        ...
        if (policy.getFlags$fragment_release().contains(Flag.PENALTY_LOG)) {
            Log.d(TAG, Intrinsics.stringPlus("Policy violation in ", fragmentName), violation);
        }
        if (policy.getListener() != null) {
            runOnHostThread(fragment, new Runnable() { // from class: androidx.fragment.app.strictmode.FragmentStrictMode$$ExternalSyntheticLambda0
                ...
                public final void run() {
                    FragmentStrictMode.m43handlePolicyViolation$lambda0(FragmentStrictMode.Policy.this, violation);
                }
            });
        }
        if (policy.getFlags$fragment_release().contains(Flag.PENALTY_DEATH)) {
            runOnHostThread(fragment, new Runnable() { // from class: androidx.fragment.app.strictmode.FragmentStrictMode$$ExternalSyntheticLambda1
                @Override // java.lang.Runnable
                public final void run() {
                    FragmentStrictMode.m44handlePolicyViolation$lambda1(fragmentName, violation);
                }
            });
        }
    }
    ...
    public static final void m43handlePolicyViolation$lambda0(Policy policy, Violation violation) {
    ...
    public static final void m44handlePolicyViolation$lambda1(String $fragmentName, Violation violation) {
        Intrinsics.checkNotNullParameter(violation, "$violation");
        Log.e(TAG, Intrinsics.stringPlus("Policy violation with PENALTY_DEATH in ", $fragmentName), violation);
        throw violation;
    }
    ...
    public static final class Policy {
        ...
        /* JADX WARN: Multi-variable type inference failed */
        public Policy(Set<? extends Flag> flags, OnViolationListener listener, Map<Class<? extends Fragment>, ? extends Set<Class<? extends Violation>>> allowedViolations) {
