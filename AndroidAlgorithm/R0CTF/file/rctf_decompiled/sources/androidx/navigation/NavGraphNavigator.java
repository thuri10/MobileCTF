...
public class NavGraphNavigator extends Navigator<NavGraph> {
    ...
    private final void navigate(NavBackStackEntry entry, NavOptions navOptions, Navigator.Extras navigatorExtras) {
        ...
        NavGraph destination = (NavGraph) entry.getDestination();
        Bundle args = entry.getArguments();
        int startId = destination.getStartDestId();
        String startRoute = destination.getStartDestinationRoute();
        if (!((startId == 0 && startRoute == null) ? false : true)) {
            throw new IllegalStateException(Intrinsics.stringPlus("no start destination defined via app:startDestination for ", destination.getDisplayName()).toString());
        }
        if (startRoute != null) {
            startDestination = destination.findNode(startRoute, false);
        } else {
            startDestination = destination.findNode(startId, false);
        }
