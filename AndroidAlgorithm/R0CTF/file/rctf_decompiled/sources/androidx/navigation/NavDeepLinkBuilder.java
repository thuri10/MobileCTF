...
public final class NavDeepLinkBuilder {
    ...
    private final void fillInIntent() {
        ...
        for (DeepLinkDestination destination : this.destinations) {
            ...
            int[] buildDeepLinkIds = node.buildDeepLinkIds(previousDestination);
            int i = 0;
            int length = buildDeepLinkIds.length;
            while (i < length) {
                int id = buildDeepLinkIds[i];
                i++;
    ...
    public final PendingIntent createPendingIntent() {
        ...
        if (globalArgs != null) {
            for (String key : globalArgs.keySet()) {
                ...
                requestCode = (requestCode * 31) + (value == null ? 0 : value.hashCode());
            }
        }
        for (DeepLinkDestination destination : this.destinations) {
            int destId = destination.getDestinationId();
            ...
            if (arguments != null) {
                for (String key2 : arguments.keySet()) {
                    ...
                    requestCode = (requestCode * 31) + (value2 == null ? 0 : value2.hashCode());
                }
            }
        }
