...
public final class TestTimeSource extends AbstractLongTimeSource {
    ...
    public final void m1581plusAssignLRDsOJo(long duration) {
        long newReading;
        long longDelta = Duration.m1493toLongimpl(duration, getUnit());
        if (longDelta != Long.MIN_VALUE && longDelta != Long.MAX_VALUE) {
            long j = this.reading;
            newReading = j + longDelta;
            if ((j ^ longDelta) >= 0 && (j ^ newReading) < 0) {
                m1580overflowLRDsOJo(duration);
            }
        } else {
            double delta = Duration.m1490toDoubleimpl(duration, getUnit());
    ...
    private final void m1580overflowLRDsOJo(long duration) {
        throw new IllegalStateException("TestTimeSource will overflow if its reading " + this.reading + "ns is advanced by " + ((Object) Duration.m1496toStringimpl(duration)) + '.');
    }
}
