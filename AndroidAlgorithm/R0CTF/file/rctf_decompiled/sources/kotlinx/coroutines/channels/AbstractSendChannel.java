...
public abstract class AbstractSendChannel<E> implements SendChannel<E> {
    ...
    public final Object mo1623trySendJP2dKIU(E element) {
        Object result = offerInternal(element);
        if (result == AbstractChannelKt.OFFER_SUCCESS) {
            return ChannelResult.INSTANCE.m1643successJP2dKIU(Unit.INSTANCE);
        }
        if (result == AbstractChannelKt.OFFER_FAILED) {
            Closed closedForSend = getClosedForSend();
            return closedForSend == null ? ChannelResult.INSTANCE.m1642failurePtdJZtk() : ChannelResult.INSTANCE.m1641closedJP2dKIU(helpCloseAndGetSendException(closedForSend));
        }
        if (result instanceof Closed) {
            return ChannelResult.INSTANCE.m1641closedJP2dKIU(helpCloseAndGetSendException((Closed) result));
        }
    ...
    public final Object sendSuspend(E e, Continuation<? super Unit> continuation) {
        ...
        while (true) {
            ...
            if (offerResult == AbstractChannelKt.OFFER_SUCCESS) {
                ...
                cont.resumeWith(Result.m132constructorimpl(Unit.INSTANCE));
                break;
            }
    ...
    public final void helpCloseAndResumeWithSendException(Continuation<?> continuation, E e, Closed<?> closed) {
        ...
        if (function1 != null && (it = OnUndeliveredElementKt.callUndeliveredElementCatchingException$default(function1, e, null, 2, null)) != null) {
            ...
            Result.Companion companion = Result.INSTANCE;
            continuation.resumeWith(Result.m132constructorimpl(ResultKt.createFailure(it)));
        } else {
            Result.Companion companion2 = Result.INSTANCE;
            continuation.resumeWith(Result.m132constructorimpl(ResultKt.createFailure(sendException)));
        }
    }
    ...
    */
    ...
    private final void helpClose(Closed<?> closed) {
        Object closedList = InlineList.m1659constructorimpl$default(null, 1, null);
        while (true) {
            LockFreeLinkedListNode prevNode = closed.getPrevNode();
            Receive previous = prevNode instanceof Receive ? (Receive) prevNode : null;
            if (previous == null) {
                break;
            } else if (!previous.mo1667remove()) {
                previous.helpRemove();
            } else {
                closedList = InlineList.m1664plusFjFbRPM(closedList, previous);
            }
        }
        if (closedList != null) {
